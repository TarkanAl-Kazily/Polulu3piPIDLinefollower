...
float proportional=0;   // The error * the proportional gain (KP)
float integral=0;       // The error summed continuosly * the integral ratio (KI)
...
#define SPEEDRATIO 1  //the ratio to convert the output to speed
...
//  OrangutanLCD::clear();
//  OrangutanLCD::print(linePosition);

  maximum = 80;

  error = linePosition - SETPOINT; // -2000 to 2000 range
  proportional = error * KP;       // -140 to 140 range
  integral = integral + (error * KI); // -maximum to maximum range (after clipping)

  // Keeps the integral under a cap to prevent infinite spinning in circles
  if (integral > maximum) {
    integral = maximum;
  }
  else if (integral < -maximum) {
    integral = -maximum;
  }

  derivative = AVERAGEERRORDELTA * KD;  // -3400.0 to 3400.0

  output = proportional + integral + derivative;  // Floating point value for motor speed
  ...
  OrangutanMotors::setSpeeds(maximum + output, maximum - output); //slows the left motor and speeds up the right motor to cause it to turn left
}
